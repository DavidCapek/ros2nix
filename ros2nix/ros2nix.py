...
DARWIN_STUBS = [
    ("lttng-tools",    "lttng-tools-stub"),
    ("lttng-ust",      "lttng-ust-stub"),
    ("lttng-ust_2_12", "lttng-ust-2_12-stub"),
]

def _darwin_stub_overlay_nix() -> str:
    stubs = "\n".join(
        f'    {name} = prev.runCommand "{stub}" {{}} "mkdir -p $out";'
        for name, stub in DARWIN_STUBS
    )
    return (
...
def generate_shell(args, packages: set[str], our_cmd_line: str):
    nix_ros_overlay = flakeref_to_expr(args.nix_ros_overlay)
    if args.darwin:
        stub = _darwin_stub_overlay_nix()
        pkgs_default = (
            f"import nix-ros-overlay {{\n"
            f"  config = {{ allowUnsupportedSystem = true; allowBroken = true; }};\n"
            f"  overlays = [ {stub} ];\n"
            f"}}"
        )
    else:
        pkgs_default = "import nix-ros-overlay { }"
...
{indent(chr(10).join(sorted(list(packages))), "            ")}
          ]
        )
...
def _darwin_flake_config(args) -> str:
    if not args.darwin:
        return (
            "{\n"
            "              overlays = [\n"
...
{indent(ros_distro_overlays_def(ros_sources), "        ")}
        pkgs = import nixpkgs ({{
          inherit system;
        }} // ({_darwin_flake_config(args)} if True else {{}}));
        rosDistro = "{args.distro}";
'''
            + '''
      in {
...
def ros2nix(args):
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Generate Darwin (macOS)-compatible shell.nix and flake.nix. "
            "Stubs out Linux-only packages (lttng-tools, lttng-ust) and "
            "sets allowUnsupportedSystem=true."
        ),
    )
